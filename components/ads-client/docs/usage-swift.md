...
- **`build()`** - Builds and returns the configured client. **Single-use:** `build()` moves the `telemetry` and context ID provider implementations into the client rather than copying them, so that a builder still awaiting release cannot keep those objects alive past shutdown. Calling `build()` a second time on the same builder returns a client with no-op telemetry and no context ID provider.
